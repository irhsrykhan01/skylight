...
async function dispatchHook(hookName, payload) {
  ...
  for (const hook of hooks) {
    try {
      ...
      if (result === false) return false; // Batalkan proses jika hook mereturn false
    } catch (err) {
...
/**
 * HTTP Client inti dengan skema retry, timeout, dan validasi respons
 */
async function httpRequest(endpoint, url, platform, retries = 2, timeoutMs = 15000) {
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    ...
    try {
      ...
      // Jeda 1 detik sebelum mencoba kembali (retry)
      if (attempt < retries) {
  ...
  // Hook: onDownloadError
  await dispatchHook('onDownloadError', { url, platform, error: lastError });

  throw lastError;
}

// Pusat penanganan error visual ramah pengguna
